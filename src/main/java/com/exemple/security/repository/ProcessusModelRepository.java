...
public interface ProcessusModelRepository extends JpaRepository<ProcessusModel, Long>{
	...
	@Query("select "
			...
			+ " else false end "
			+ " from ProcessusModel p "
			+ " where p.orderPM = :val  and p.statut in('0','1')")
	boolean existsByOrderProADD(@Param("val") Integer val);
	...
	@Query("select "
			...
			+ " else false end "
			+ " from ProcessusModel p "
			+ " where p.orderPM = :val and  p.statut not in('-1','-2') "
			+ " and p.id <> :id")
	boolean existsByOrderProMod(@Param("val") Integer val , @Param("id") Long id);
}
